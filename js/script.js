...
$(function () {
    ...
    new ScrollMagic.Scene({triggerElement: "#chatbox", duration: 1000, reverse: true})
                    ...
                    .on("progress", function (event) {
                        if(event.target.controller().info("scrollDirection")=="FORWARD") {
                            for(var i=0; i<event.progress/0.1; i++) {
...
app.directive('myEnter', function () {
    return function (scope, element, attrs) {
        element.bind("keydown keypress", function (event) {
            if(event.which === 13) {
                if($('#talk-input-text').val().trim()!="") {
                    var targetDiv;
                    targetDiv = $(".talk-bubble:visible:last");
                    $($(".talk-bubble")[0]).clone().insertAfter(targetDiv).addClass('btm-right-in').html($('#talk-input-text').val()).show();
                    $('#talk-input-text').val('');
                    targetDiv = $(".talk-bubble:visible:last");
                    $($(".talk-bubble")[0]).clone().insertAfter(targetDiv).addClass('btm-left-in').html('The message will not send to me. Please email to contact!').show();
                    $(".talk-log").animate({ scrollTop: $(".talk-log")[0].scrollHeight }, 1000);
                }
